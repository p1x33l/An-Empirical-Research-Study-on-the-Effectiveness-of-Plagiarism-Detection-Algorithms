...
echo"<h1> text :</h1>  ";
    $text = "putsch vient d’échapper une nouvelle fois aux sanctions dont la menaçait la Cédéao";
    echo $text."<br>";
    ...
    $text_nostopword = removeStopWordsFr($text);
    echo "<br>";
    echo "<br>";
    echo "<br>";
        //----------------------------------------normalisation------------------------------------
        function normalisation($text_nostopword){
            global $stop_words;
            $input=strtolower($text_nostopword);
            //$text_without_Punctuation = preg_replace('/\p{P}/', '',$input);
            $text_without_Punctuation=str_replace([',',';',':','?', '!', '.','«','»','-','- -','”','“','«','»',' "','"','...','{','}','(',')','[',']','+','-','*','/','&','|','<','>','=','~','@','$','\\'], '', $text_nostopword);
            $text_without_number = preg_replace('/[0-9]+/', '', $text_without_Punctuation);
            $text_normalize =  $text_without_number;
            return $text_normalize;
        }
        echo"<h1> text normalize: </h1> ";
        echo normalisation($text_nostopword)."<br>";
        echo "<br>";
    ...
    function stemming($input){
        global $stop_words;
        $text_stemming = "";
        $input_normalize = normalisation($input);
        $input = removeStopWordsFr($input_normalize);
